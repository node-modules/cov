...
module.exports = function istanbulDir(threshold, client, root, pkg) {
  ...
  files.forEach(function (filename, i) {
    var name = 'cov/html/node-' + process.version + '/'
      + pkg.name + '/' + pkg.version + filename.replace(lcovDir, '');
    client.delete(name, function (err) {
      client.uploadFile(filename, {key: name}, function (err, info) {
        done++;
        if (err) {
          console.log('Save %s error: %s', name, err);
        }
        if (filename && filename.indexOf('/lcov-report/index.html') >= 0) {
          indexURL = info.url;
        }
